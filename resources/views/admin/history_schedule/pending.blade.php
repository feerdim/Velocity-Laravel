@extends('layouts.app', ['title' => 'Pending Game', 'link' => 'admin.history_schedule.pending'])
        ...
        <div class="section-body">
            ...
            <div class="card">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h4><i class="fas fa-users"></i> Pending Games</h4>
                </div>
                    ...
                    <div class="">
                        <table class="table-responsive table table-bordered table-flush" id="datatable-basic">
                            ...
                            <tbody>
                                ...
                                @endphp --}}
                                @php
                                    $group_by_game = $pendings->unique(function ($item) {
                                ...
                                @forelse ($group_by_game as $index=> $history)
                                    <tr style="text-align: center">
                                        ...
                                        {{-- <td>{{ $totalPlayer[$index]->total }}</td> --}}
                                        {{-- <td>
                                            {{ $history->player->activations->where('player_id', $history->player->id)->where('game_id', $history->game->id)->first()->account_id }}
                                        </td>
                                        <td>
                                            {{ $history->player->activations->where('player_id', $history->player->id)->where('game_id', $history->game->id)->first()->account_name }}
                                        </td> --}}
